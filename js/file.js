...
// ********* RECT *********
  ...
  $("#save").click(function() {
    // console.log(JSON.stringify(drawing.shapes));
    var stringifiedArray = JSON.stringify(drawing.shapes);
    var param = { 
      "user": "sigurdura13",
      "name": $("#save-imagename").val() || "No Name",
  ...
  $.ajax({
    ...
    data: {user: "sigurdura13", template: true},
    dataType: "jsonp",
    crossDomain: true,
    success: function (data) {
      var load = $("#load");
      data.forEach(function(picture){
        console.log(picture);
        load.append('<option value="'+ picture.WhiteboardContents.replace(/"/g, '&quot;') +'">' + picture.WhiteboardTitle + "</option>")
      });
      load.on("change", function(){
        console.log($(this).val());
        drawing = JSON.parse($(this).val());
      })
    },
    error: function (xhr, err) {
      console.log(err);
